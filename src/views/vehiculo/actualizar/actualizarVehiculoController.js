...
export const actualizarVehiculoControllador = async (parametros = null) => {
    ...
    const ingresoExiste = await obtenerDatos(`detallesingreso/vehiculo/${data.id}`);
    console.log(ingresoExiste);
    

    if (ingresoExiste.code == 200) {
        idPiloto = ingresoExiste.data.id_duenio_vehiculo;
        servicioVeh.value = ingresoExiste.data.codigo_servicio;


        const piloto = await obtenerDatos(`propietariosvehiculos/id/${idPiloto}`);
        console.log(piloto);
    ...
    try {
        ...
        const registroIngreso = await editarDato(`detallesingreso`, ingresoExiste.data.id_registro, ingreso)
